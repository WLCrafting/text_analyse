...
def main():
    ...
    # 以下生成历史交易数据
    # trade_history(start = '2017-5-10', province = '广东')
    # 以上生成历史交易 用#开关, 

    # 以下生成历史政策数据
    # policy_sql_condition = policy_set_sql_conditon(start = '2016-5-10', province = '江西')
    # res = fetch_policy(policy_sql_condition)
    # pd.DataFrame(res).to_csv('policy_result.csv', mode = 'w', header = False, index = True, encoding = 'gbk')
    # 以上生成历史政策数据

    # 以下生成公司有关数据
    # 可以用名字模糊查询出ID
    # res = fetch_company(company_name = '新奥')
    # 利用ID查询结果
    res = fetch_company(company_id = 265)
